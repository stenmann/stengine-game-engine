...
Idle = function(self)
    if self.boss.hp > 50 then
        if self.state_timer >= 180 and self.hand1.charging == false then
            self:SetState("charge_in")
        end
    end

    if self.boss.hp <= 50 then
        if self.state_timer >= 180 and self.hand1.charging == false and self.lastAttack == "laser" then
            self:SetState("charge_in")
            self.lastAttack = "charge_in"
        elseif self.state_timer >= 180 and self.hand1.charging == false and self.lastAttack == "charge_in" then
            self:SetState("laser")
    ...
    LaserAttack = function(self)
        ...
        if not self.laser_ready then
            if math.abs(self.transform.y - self.laser_target_y) > 0.1 then
                ...
                self.laser_t.rotation_degrees = 180
